...
<body class="em_body" style="margin: 0px auto; padding: 0px;" bgcolor="#efefef">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="em_full_wrap" align="center" bgcolor="#efefef">
    <tr>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="em_full_wrap" align="center" bgcolor="#efefef">
        <tr>
          <td height="2" style="height: 20px; font-size: 1px; line-height: 1px;">
            &nbsp;
          </td>
        </tr>
      </table>
...
export default async function contactUsEmail(email, subject, message) {
  const newEmailTemplate = emailIs.replace('{{email}}', email).replace('{{subject}}', subject).replace('{{message}}', message);
  const mailedData = await notification.sendEmail(Locals.contactEmail, `${email} sent a mail from wantJSON contact us page !`, newEmailTemplate);
  return mailedData;
}
