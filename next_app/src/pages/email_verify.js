...
export default function EmailVerify() {
  ...
  const onResend = async () => {
    const response = await axios('send_otp', 'post', { email: emailInput, type: 'email_verify' });
    if (response.success) {
    ...
    <AuthPage>
      <div className="form-box">
        ...
        <div className="form-box-bottom">
          <button type="button" className="button form-text" onClick={() => onResend()} disabled={resendDisable}>Resend Email</button>
        </div>
      </div>
    </AuthPage>
  );
